...
4. ✅ Realistic amounts based on actual pricing (from `subscription.service.ts` PRICING constants):
   - PREMIUM Monthly: €599.00 (59900 cents / 100)
   - PREMIUM Annual: €6,469.20 (646920 cents / 100)
   - ENTERPRISE Monthly: €1,999.00 (199900 cents / 100)
   - ENTERPRISE Annual: TBD (check PRICING.ENTERPRISE_ANNUAL.price)
...
- [ ] Task 4: Generate Invoice Data (AC: 3, 4)
  ...
  - [ ] Import PRICING constants from `../src/services/subscription.service.ts`
  - [ ] Calculate amount based on subscription plan and billingCycle:
    - PREMIUM + MONTHLY → PRICING.PREMIUM_MONTHLY.price / 100 = €599.00
    - PREMIUM + ANNUAL → PRICING.PREMIUM_ANNUAL.price / 100 = €6,469.20
    - ENTERPRISE + MONTHLY → PRICING.ENTERPRISE_MONTHLY.price / 100 = €1,999.00
    - ENTERPRISE + ANNUAL → PRICING.ENTERPRISE_ANNUAL.price / 100
...
# Expected output (amounts will vary based on random plan/cycle distribution):
# Seeding revenue data...
# Created 50 invoices across 8 organizations
# - 40 PAID invoices (€35,460)
# - 5 OPEN invoices (€4,995)
# - 3 VOID invoices (€2,997)
# - 2 DRAFT invoices (€1,998)
# Total test revenue: €35,460
# Note: Amounts higher due to correct PRICING constants (€599-€6,469 range)
...
**Key Logic**:
```typescript
// Amount calculation - IMPORTANT: PRICING constants are in CENTS
// Invoice.amount must be in EUROS (Float), so divide by 100
