...
- [ ] Task 2: Create User Stats Endpoint (AC: 2, 3)
  - [ ] Add `getUserStats` method to `backend/src/services/user.service.ts`
    - [ ] Query total user count excluding DELETED status
    - [ ] Query active users (status = ACTIVE)
    - [ ] Query verified users (emailVerified = true)
    - [ ] Group by role for byRole counts
    - [ ] Group by status for byStatus counts
  - [ ] Add `GET /admin/users/stats` route to `backend/src/routes/admin.routes.ts`
  - [ ] Add Zod schema for stats response
  - [ ] Add authentication and ADMIN role check
  - [ ] Test endpoint returns correct aggregated data
