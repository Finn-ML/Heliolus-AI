...
async function checkOptimizationStatus() {
  try {
    ...
    if (process.env.AI_ENABLE_PREPROCESSING === 'true') {
      ...
      console.log('');
      console.log('🚀 Expected Performance for Next Assessment:');
      if (documentCount > 0) {
        console.log(`   With ${documentCount} documents × 50 questions:`);
        const legacy = documentCount * 50;
        const optimized = documentCount + 50;
        const savings = Math.round(((legacy - optimized) / legacy) * 100);
        console.log(`     Legacy: ${legacy} API calls, ~${Math.round(legacy * 0.6)}s`);
        console.log(`     Optimized: ${optimized} API calls, ~${Math.round(optimized * 0.16)}s`);
        console.log(`     ⚡ ${Math.round(legacy * 0.6 / (optimized * 0.16))}x faster, ${savings}% fewer API calls`);
      }
    } else {
